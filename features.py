# File: src/features.py (FINAL CORRECTED LOGIC - Divided)
# File: src/features.py (FINAL CORRECTED LOGIC)
...
# --- GLOBAL MODEL INITIALIZATION (LOADED ONCE) ---
@torch.no_grad()
def initialize_models(): # <--- SYNCHRONIZED FUNCTION NAME
...
# The function is called once inside main.py and the results are passed.
# The code below is NOT executed upon import anymore, the function is just defined.
...
def extract_structured_features(df, target_col='price', le_brand=None, le_ipq=None):
    """
    Extracts Brand, IPQ, handles target price normalization, and ensures 1D array output 
    for LabelEncoder to prevent ValueError.
    """
    
    # Create copy and immediately reset index for chunk safety
    ...
    df_copy.reset_index(drop=True, inplace=True) # Reset after concat for safety
    ...
    if le_brand is None:
        ...
        df_copy['Brand_Encoded'] = [
            le_brand.transform([str(x)])[0] 
            if str(x) in classes_list 
            else unknown_label 
            for x in df_copy['Brand'].values
        ]
    ...
    if le_ipq is None:
        ...
        unknown_label = len(classes_list)
        df_copy['IPQ_Unit_Encoded'] = [
            le_ipq.transform([str(x)])[0] 
            if str(x) in classes_list 
            else unknown_label 
            for x in df_copy['IPQ_Unit'].values
        ]

    y_train = None
    if target_col in df_copy.columns:
        # Use NumPy arrays and force 1D shape to avoid index/broadcast errors
        price_values = df_copy[target_col].to_numpy()
        ipq_values = df_copy['IPQ_Value'].to_numpy()
        
        # Check if ipq_values is 2D and needs reshaping 
        ...
        # Final Stability Fix: Clamp IPQ values to prevent division by zero / NaN
